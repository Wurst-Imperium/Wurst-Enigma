...
public class TestJarIndexLoneClass
{
	...
	{
		assertThat(
			m_index.getTranslationIndex().getSuperclass(
				new ClassEntry("none/a")), is(nullValue()));
		assertThat(
			m_index.getTranslationIndex().getSuperclass(
				new ClassEntry("cuchaz/enigma/inputs/Keep")), is(nullValue()));
		assertThat(
			m_index.getTranslationIndex().getAncestry(new ClassEntry("none/a")),
			is(empty()));
		assertThat(
			m_index.getTranslationIndex().getAncestry(
				new ClassEntry("cuchaz/enigma/inputs/Keep")), is(empty()));
		assertThat(
			m_index.getTranslationIndex().getSubclass(new ClassEntry("none/a")),
			is(empty()));
		assertThat(
	...
	{
		assertThat(
			m_index.getAccess(newField("none/a", "a", "Ljava/lang/String;")),
			is(Access.Private));
		assertThat(
			m_index.getAccess(newMethod("none/a", "a", "()Ljava/lang/String;")),
			is(Access.Public));
		assertThat(
			m_index.getAccess(newField("none/a", "b", "Ljava/lang/String;")),
			is(nullValue()));
	...
	public void classInheritance()
	{
		ClassInheritanceTreeNode node =
			m_index.getClassInheritance(new Translator(), newClass("none/a"));
	...
	public void methodInheritance()
	{
		...
		MethodInheritanceTreeNode node =
			m_index.getMethodInheritance(new Translator(), source);
		assertThat(node, is(not(nullValue())));
	...
	public void classImplementations()
	{
		ClassImplementationsTreeNode node =
			m_index.getClassImplementations(new Translator(),
				newClass("none/a"));
	...
	public void relatedMethodImplementations()
	{
		Set<MethodEntry> entries =
			m_index.getRelatedMethodImplementations(newMethod("none/a", "a",
				"()Ljava/lang/String;"));
		assertThat(
			entries,
	...
	public void fieldReferences()
	{
		...
		Collection<EntryReference<FieldEntry, BehaviorEntry>> references =
			m_index.getFieldReferences(source);
		assertThat(
			references,
			containsInAnyOrder(
				newFieldReferenceByConstructor(source, "none/a",
					"(Ljava/lang/String;)V"),
	...
	public void contains()
	{
		...
		assertThat(m_index.containsObfField(newField("none/a", "a",
			"Ljava/lang/String;")), is(true));
		assertThat(m_index.containsObfField(newField("none/a", "b",
			"Ljava/lang/String;")), is(false));
		assertThat(m_index.containsObfField(newField("none/a", "a", "LFoo;")),
			is(false));
		assertThat(m_index.containsObfBehavior(newMethod("none/a", "a",
			"()Ljava/lang/String;")), is(true));
		assertThat(m_index.containsObfBehavior(newMethod("none/a", "b",
			"()Ljava/lang/String;")), is(false));
	}
}
