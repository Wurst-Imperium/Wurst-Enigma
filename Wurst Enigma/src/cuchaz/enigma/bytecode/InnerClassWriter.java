...
public class InnerClassWriter
{
	...
	public void write(CtClass c)
	{
		...
		List<ClassEntry> obfClassChain =
			m_index.getObfClassChain(obfClassEntry);
		...
		if(isInnerClass)
		{
			...
			BehaviorEntry caller =
				m_index.getAnonymousClassCaller(obfClassEntry);
			if(caller != null)
				// write the enclosing method attribute
				if(caller.getName().equals("<clinit>"))
					c.getClassFile().addAttribute(
						new EnclosingMethodAttribute(c.getClassFile()
							.getConstPool(), caller.getClassName()));
				else
					c.getClassFile().addAttribute(
						new EnclosingMethodAttribute(c.getClassFile()
							.getConstPool(), caller.getClassName(), caller
							.getName(), caller.getSignature().toString()));
		}

		// does this class have any inner classes?
		Collection<ClassEntry> obfInnerClassEntries =
			m_index.getInnerClasses(obfClassEntry);
		...
		if(isInnerClass || !obfInnerClassEntries.isEmpty())
		{
			...
			InnerClassesAttribute attr =
				new InnerClassesAttribute(c.getClassFile().getConstPool());
			...
			for(int i = 1; i < obfClassChain.size(); i++)
			{
				...
				c.replaceClassName(obfInnerClassEntry.getName(),
					obfInnerClassEntry.buildClassEntry(obfClassChain).getName());
			}
			...
			for(ClassEntry obfInnerClassEntry : obfInnerClassEntries)
			{
				...
				List<ClassEntry> extendedObfClassChain =
					Lists.newArrayList(obfClassChain);
				...
				// update references to use the fully qualified inner class name
				c.replaceClassName(obfInnerClassEntry.getName(),
					obfInnerClassEntry.buildClassEntry(extendedObfClassChain)
						.getName());
			}
		}
	}
