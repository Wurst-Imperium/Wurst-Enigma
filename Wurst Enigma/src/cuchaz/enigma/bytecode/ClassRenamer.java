...
public class ClassRenamer
{
	...
	public static void renameClasses(CtClass c, ClassNameReplacer replacer)
	{
		...
		{
			methodInfo.setDescriptor(Descriptor.rename(
				methodInfo.getDescriptor(), map));
		...
		{
			fieldInfo.setDescriptor(Descriptor.rename(
				fieldInfo.getDescriptor(), map));
		...
		if(attr != null)
			for(int i = 0; i < attr.tableLength(); i++)
			{
				 ...
				 * , classEntry, attr.outerClass(i), attr.innerClass(i),
				 * attr.innerName(i)));
				 */
			}
	}
		...
		ReplacerClassMap map, SignatureType type)
	{
		try
		{
			...
			for(AttributeInfo attribute : attributes)
				...
				{
					// this has to be handled specially because
					// SignatureAttribute.renameClass() is buggy as hell
					SignatureAttribute signatureAttribute =
						(SignatureAttribute)attribute;
					String newSignature =
						type.rename(signatureAttribute.getSignature(), map);
					if(newSignature != null)
						signatureAttribute.setSignature(newSignature);
				}else if(attribute instanceof CodeAttribute)
				{
					// code attributes have signature attributes too
					// (indirectly)
				...
				}else if(attribute instanceof LocalVariableTypeAttribute)
				{
					...
					LocalVariableTypeAttribute localVariableAttribute =
						(LocalVariableTypeAttribute)attribute;
					renameLocalVariableTypeAttribute(localVariableAttribute,
						map);
				}else
		...
		{
			throw new Error("Unable to call javassist methods by reflection!",
				ex);
		}
	}
		...
		LocalVariableTypeAttribute attribute, ReplacerClassMap map)
	{
		...
		for(int i = 0; i < n; ++i)
		{
			...
			if(index != 0)
			{
				...
				String newSignature =
					renameLocalVariableSignature(signature, map);
				if(newSignature != null)
					ByteArray.write16bit(cp.addUtf8Info(newSignature), info,
						pos + 6);
			}
		}
	}
	...
	private static String renameClassSignature(String signature,
		ReplacerClassMap map)
	{
		try
		{
			ClassSignature type =
				renameType(SignatureAttribute.toClassSignature(signature), map);
			if(type != null)
				return type.encode();
			return null;
		...
		ReplacerClassMap map)
	{
		...
		{
			ObjectType type =
				renameType(SignatureAttribute.toFieldSignature(signature), map);
			if(type != null)
				return type.encode();
			return null;
		}catch(BadBytecode ex)
		...
		ReplacerClassMap map)
	{
		try
		{
			MethodSignature type =
				renameType(SignatureAttribute.toMethodSignature(signature), map);
			if(type != null)
				return type.encode();
			return null;
		}catch(BadBytecode ex)
		...
		ReplacerClassMap map)
	{
		...
		{
			typeParamTypes =
				Arrays.copyOf(typeParamTypes, typeParamTypes.length);
		...
		ReplacerClassMap map)
	{
		...
		{
			exceptionTypes =
				Arrays.copyOf(exceptionTypes, exceptionTypes.length);
			for(int i = 0; i < exceptionTypes.length; i++)
			{
				ObjectType newExceptionType =
					renameType(exceptionTypes[i], map);
	...
	private static TypeVariable renameType(TypeVariable type,
		ReplacerClassMap map)
	{
	...
	private static String getClassName(ClassType type)
	{
		...
		{
			NestedClassType nestedType = (NestedClassType)type;
			return getClassName(nestedType.getDeclaringClass()) + "$"
				+ Descriptor.toJvmName(type.getName());
		}else
	...
	private static TypeArgument renameType(TypeArgument type,
		ReplacerClassMap map)
	{
