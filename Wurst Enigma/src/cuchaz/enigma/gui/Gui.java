...
public class Gui
{
	...
	public Gui()
	{
		...
		if(Boolean.parseBoolean(System.getProperty("enigma.catchExceptions",
			"true")))
		{
		...
		m_obfClasses =
			new ClassSelector(ClassSelector.ObfuscatedClassEntryComparator);
		...
		m_splitClasses =
			new JSplitPane(JSplitPane.VERTICAL_SPLIT, true, obfPanel,
				deobfPanel);
		...
		m_infoPanel.setBorder(BorderFactory
			.createTitledBorder("Identifier Info"));
		clearReference();
		...
		m_inheritanceTree.addMouseListener(new MouseAdapter()
		{
			...
			public void mouseClicked(MouseEvent event)
			{
				if(event.getClickCount() == 2)
				{
					...
					{
						ClassInheritanceTreeNode classNode =
							(ClassInheritanceTreeNode)node;
		...
		m_callsTree.addMouseListener(new MouseAdapter()
		{
			...
			public void mouseClicked(MouseEvent event)
			{
				if(event.getClickCount() == 2)
				{
					...
					{
						ReferenceTreeNode<Entry, Entry> referenceNode =
							(ReferenceTreeNode<Entry, Entry>)node;
		...
		JSplitPane callPanel =
			new JSplitPane(JSplitPane.VERTICAL_SPLIT, true, new JScrollPane(
				m_callsTree), new JScrollPane(m_tokens));
		...
		JSplitPane splitRight =
			new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, centerPanel,
				m_tabs);
		splitRight.setResizeWeight(1); // let the left side take all the slack
		splitRight.resetToPreferredSizes();
		JSplitPane splitCenter =
			new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, m_classesPanel,
				splitRight);
				...
				item.addActionListener(new ActionListener()
				{
					...
					public void actionPerformed(ActionEvent event)
					{
						if(m_mappingsFileChooser.showOpenDialog(m_frame) == JFileChooser.APPROVE_OPTION)
							try
							{
								m_controller.openMappings(m_mappingsFileChooser
									.getSelectedFile());
							}catch(IOException ex)
							{
								throw new Error(ex);
			...
			}
			{
				...
				item.addActionListener(new ActionListener()
				{
					...
					public void actionPerformed(ActionEvent event)
					{
						try
						{
							m_controller.saveMappings(m_mappingsFileChooser
								.getSelectedFile());
						}catch(IOException ex)
						{
							throw new Error(ex);
				...
				item.addActionListener(new ActionListener()
				{
					...
					public void actionPerformed(ActionEvent event)
					{
						if(m_exportSourceFileChooser.showSaveDialog(m_frame) == JFileChooser.APPROVE_OPTION)
							m_controller.exportSource(m_exportSourceFileChooser
								.getSelectedFile());
					}
				});
				...
				item.addActionListener(new ActionListener()
				{
					...
					public void actionPerformed(ActionEvent event)
					{
						if(m_exportJarFileChooser.showSaveDialog(m_frame) == JFileChooser.APPROVE_OPTION)
							m_controller.exportJar(m_exportJarFileChooser
								.getSelectedFile());
					}
				});
	...
	public void showToken(final Token token)
	{
		...
		CodeReader
			.navigateToToken(m_editor, token, m_selectionHighlightPainter);
		redraw();
	}
	...
	public void setHighlightedTokens(Iterable<Token> obfuscatedTokens,
		Iterable<Token> deobfuscatedTokens, Iterable<Token> otherTokens)
	{
		...
		if(deobfuscatedTokens != null)
			setHighlightedTokens(deobfuscatedTokens,
				m_deobfuscatedHighlightPainter);
		if(otherTokens != null)
	...
	private void setHighlightedTokens(Iterable<Token> tokens,
		Highlighter.HighlightPainter painter)
	{
		for(Token token : tokens)
			try
			{
				m_editor.getHighlighter().addHighlight(token.start, token.end,
					painter);
	...
	private void showReference(EntryReference<Entry, Entry> reference)
	{
		...
		if(reference.entry instanceof ClassEntry)
			...
			throw new Error("Unknown entry type: "
				+ m_reference.entry.getClass().getName());
	...
	{
		addNameValue(m_infoPanel, "Constructor", entry.getClassEntry()
			.getName());
		if(!entry.isStatic())
	...
	private void addNameValue(JPanel container, String name, String value)
	{
		...
		label.setPreferredSize(new Dimension(100,
			label.getPreferredSize().height));
		panel.add(label);
		
		panel
			.add(GuiTricks.unboldLabel(new JLabel(value, SwingConstants.LEFT)));
	}
	...
	private void onCaretMove(int pos)
	{
		...
		boolean isClassEntry =
			isToken && m_reference.entry instanceof ClassEntry;
		boolean isFieldEntry =
			isToken && m_reference.entry instanceof FieldEntry;
		boolean isMethodEntry =
			isToken && m_reference.entry instanceof MethodEntry;
		boolean isConstructorEntry =
			isToken && m_reference.entry instanceof ConstructorEntry;
		boolean isInJar =
			isToken && m_controller.entryIsInJar(m_reference.entry);
		boolean isRenameable =
			isToken && m_controller.referenceIsRenameable(m_reference);
		...
		m_showCallsMenu.setEnabled(isClassEntry || isFieldEntry
			|| isMethodEntry || isConstructorEntry);
		m_openEntryMenu
			.setEnabled(isInJar
				&& (isClassEntry || isFieldEntry || isMethodEntry || isConstructorEntry));
	...
	private void finishRename(JTextField text, boolean saveName)
	{
		...
		JPanel panel = (JPanel)m_infoPanel.getComponent(0);
		panel.remove(panel.getComponentCount() - 1);
		panel.add(GuiTricks.unboldLabel(new JLabel(
			m_reference.getNamableName(), SwingConstants.LEFT)));
	...
	private void showInheritance()
	{
		...
		if(m_reference.entry instanceof ClassEntry)
		{
			...
			ClassInheritanceTreeNode classNode =
				m_controller.getClassInheritance((ClassEntry)m_reference.entry);
			
			// show the tree at the root
			TreePath path = getPathToRoot(classNode);
			m_inheritanceTree.setModel(new DefaultTreeModel((TreeNode)path
				.getPathComponent(0)));
			m_inheritanceTree.expandPath(path);
			m_inheritanceTree.setSelectionRow(m_inheritanceTree
				.getRowForPath(path));
		}else if(m_reference.entry instanceof MethodEntry)
		{
			// get the method inheritance
			MethodInheritanceTreeNode classNode =
				m_controller
					.getMethodInheritance((MethodEntry)m_reference.entry);
	...
	private void showImplementations()
	{
		...
		if(m_reference.entry instanceof ClassEntry)
		{
			...
			ClassImplementationsTreeNode node =
				m_controller
					.getClassImplementations((ClassEntry)m_reference.entry);
			if(node != null)
			{
				// show the tree at the root
				TreePath path = getPathToRoot(node);
				m_implementationsTree.setModel(new DefaultTreeModel(
					(TreeNode)path.getPathComponent(0)));
				m_implementationsTree.expandPath(path);
				m_implementationsTree.setSelectionRow(m_implementationsTree
					.getRowForPath(path));
			}
		}else if(m_reference.entry instanceof MethodEntry)
		{
			...
			MethodImplementationsTreeNode node =
				m_controller
					.getMethodImplementations((MethodEntry)m_reference.entry);
			if(node != null)
			{
				// show the tree at the root
				TreePath path = getPathToRoot(node);
	...
	private void showCalls()
	{
		...
		if(m_reference.entry instanceof ClassEntry)
		{
			...
			BehaviorReferenceTreeNode node =
				m_controller.getMethodReferences(new ConstructorEntry(
					(ClassEntry)m_reference.entry, new Signature("()V")));
			m_callsTree.setModel(new DefaultTreeModel(node));
		}else if(m_reference.entry instanceof FieldEntry)
		{
			FieldReferenceTreeNode node =
				m_controller.getFieldReferences((FieldEntry)m_reference.entry);
			m_callsTree.setModel(new DefaultTreeModel(node));
		}else if(m_reference.entry instanceof MethodEntry)
		{
			BehaviorReferenceTreeNode node =
				m_controller
					.getMethodReferences((MethodEntry)m_reference.entry);
			m_callsTree.setModel(new DefaultTreeModel(node));
		}else if(m_reference.entry instanceof ConstructorEntry)
		{
			BehaviorReferenceTreeNode node =
				m_controller
					.getMethodReferences((ConstructorEntry)m_reference.entry);
			m_callsTree.setModel(new DefaultTreeModel(node));
		}
