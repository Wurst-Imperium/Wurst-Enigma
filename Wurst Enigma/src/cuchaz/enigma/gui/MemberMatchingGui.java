...
public class MemberMatchingGui<T extends Entry>
{
	...
	public MemberMatchingGui(ClassMatches classMatches,
		...
		Deobfuscator destDeobfuscator)
	{
		...
		classesPanel
			.setLayout(new BoxLayout(classesPanel, BoxLayout.PAGE_AXIS));
		...
		JSplitPane splitRight =
			new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, new JScrollPane(
				m_sourceReader), new JScrollPane(m_destReader));
		splitRight.setResizeWeight(0.5); // resize 50:50
		JSplitPane splitLeft =
			new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, classesPanel,
				splitRight);
	...
	protected void setSourceClass(ClassEntry sourceClass)
	{
		...
		m_destReader.decompileClass(m_obfDestClass, m_destDeobfuscator, false,
			new Runnable()
			{
				@Override
				public void run()
	...
	protected void updateSourceHighlights()
	{
		highlightEntries(m_sourceReader, m_sourceDeobfuscator, m_memberMatches
			.matches().keySet(), m_memberMatches.getUnmatchedSourceEntries());
	}
	...
	protected void onSelectSource(Entry source)
	{
		...
		if(source != null)
		{
			...
			// Entry
			// and MemberMatches.hasSource() will only pass entries that
			// actually match T
	...
	protected void onSelectDest(Entry dest)
	{
		...
		if(dest != null)
		{
			...
			if(m_memberMatches.hasDest(obfDestEntry))
			{
				...
				T obfSourceEntry =
					m_memberMatches.matches().inverse().get(obfDestEntry);
	...
	private String getEntryLabel(T obfEntry, Deobfuscator deobfuscator)
	{
		...
		return String.format("%s (%s)", deobfEntry.getName(),
			obfEntry.getName());
	}
	...
	private void updateButtons()
	{
		...
		if(m_obfSourceEntry != null && m_obfDestEntry != null)
		{
			...
			else if(!m_memberMatches.isMatchedSourceEntry(m_obfSourceEntry)
				&& !m_memberMatches.isMatchedDestEntry(m_obfDestEntry))
			...
			GuiTricks.activateButton(m_unmatchableButton, "Set Unmatchable",
				new ActionListener()
				{
					@Override
					public void actionPerformed(ActionEvent event)
