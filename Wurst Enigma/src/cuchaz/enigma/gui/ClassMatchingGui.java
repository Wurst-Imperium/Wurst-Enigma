...
public class ClassMatchingGui
{
	...
	private static enum SourceType
	{
		...
		{
			JRadioButton button =
				new JRadioButton(name(), this == getDefault());
		...
		Deobfuscator sourceDeobfuscator, Deobfuscator destDeobfuscator)
	{
		...
		{
			JRadioButton button =
				sourceType.newRadio(sourceTypeListener, sourceTypeButtons);
		...
		JSplitPane splitLeft =
			new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, sourcePanel,
				new JScrollPane(m_sourceReader));
		...
		JSplitPane splitRight =
			new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, new JScrollPane(
				m_destReader), destPanel);
		splitRight.setResizeWeight(1); // let the left side take all the slack
		JSplitPane splitCenter =
			new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, splitLeft,
				splitRight);
	...
	private Collection<ClassEntry> getLikelyMatches(ClassEntry sourceClass)
	{
		...
		ClassIdentifier sourceIdentifier =
			new ClassIdentifier(m_sourceDeobfuscator.getJar(),
				m_sourceDeobfuscator.getJarIndex(), namer.getSourceNamer(),
				true);
		...
		try
		{
			...
			for(ClassEntry unmatchedDestClass : m_classMatches
				.getUnmatchedDestClasses())
			{
				ClassIdentity destIdentity =
					destIdentifier.identify(unmatchedDestClass);
				float score =
					100.0f
						* (sourceIdentity.getMatchScore(destIdentity) + destIdentity
							.getMatchScore(sourceIdentity))
						/ (sourceIdentity.getMaxMatchScore() + destIdentity
							.getMaxMatchScore());
				scoredDestClasses.add(new ScoredClassEntry(unmatchedDestClass,
					score));
			}
	...
	private void updateMatchButton()
	{
		...
		if(twoSelected)
			if(isMatched)
				GuiTricks.activateButton(m_matchButton, "Unmatch",
					new ActionListener()
					{
						@Override
						public void actionPerformed(ActionEvent event)
				...
				GuiTricks.activateButton(m_matchButton, "Match",
					new ActionListener()
					{
						@Override
						public void actionPerformed(ActionEvent event)
	...
	private void autoMatch()
	{
		...
		ClassMatching matching =
			MappingsConverter.computeMatching(m_sourceDeobfuscator.getJar(),
				m_sourceDeobfuscator.getJarIndex(),
	...
	private void pickBestDestClass()
	{
		...
		for(ClassSelectorPackageNode packageNode : m_destClasses.packageNodes())
			for(ClassSelectorClassNode classNode : m_destClasses
				.classNodes(packageNode))
			{
				if(firstClass == null)
					firstClass = classNode.getClassEntry();
				if(classNode.getClassEntry() instanceof ScoredClassEntry)
				{
					ScoredClassEntry scoredClass =
						(ScoredClassEntry)classNode.getClassEntry();
					if(bestDestClass == null
						|| bestDestClass.getScore() < scoredClass.getScore())
