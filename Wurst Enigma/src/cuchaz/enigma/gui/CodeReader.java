...
public class CodeReader extends JEditorPane
{
	...
	public CodeReader()
	{
		...
		addCaretListener(new CaretListener()
		{
			...
			public void caretUpdate(CaretEvent event)
			{
				...
				{
					Token token =
						m_sourceIndex.getReferenceToken(event.getDot());
					if(token != null)
						m_selectionListener.onSelect(m_sourceIndex
							.getDeobfReference(token));
					else
	...
	public void navigateToClassDeclaration(ClassEntry classEntry)
	{
		...
		if(token == null)
			// couldn't find the class declaration token, might be an anonymous
			// class
		...
		final Timer timer = new Timer(200, new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent event)
			{
				if(m_counter % 2 == 0)
					...
					{
						m_highlight =
							editor.getHighlighter().addHighlight(token.start,
								token.end, highlightPainter);
					}catch(BadLocationException ex)
					{
						// don't care
					}
