...
public abstract class BoxHighlightPainter implements
	Highlighter.HighlightPainter
{
		...
		JTextComponent text)
	{
		...
		if(m_fillColor != null)
		{
			...
			g.fillRoundRect(bounds.x, bounds.y, bounds.width, bounds.height, 4,
				4);
		}
	...
	protected static Rectangle getBounds(JTextComponent text, int start, int end)
	{
		try
		{
			...
			Rectangle bounds =
				text.modelToView(start).union(text.modelToView(end));
