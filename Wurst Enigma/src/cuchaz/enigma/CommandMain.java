...
public class CommandMain
{
	...
	public static class ConsoleProgressListener implements ProgressListener
	{
		...
		public void onProgress(int numDone, String message)
		{
			...
			if(isLastUpdate)
			{
				...
				System.out.println(String.format("Finished in %.1f seconds",
					elapsedSeconds));
			}
		}
	}
	...
	{
		System.out.println(String.format("%s - %s", Constants.Name,
			Constants.Version));
		System.out.println("Usage:");
		System.out
			.println("\tjava -cp enigma.jar cuchaz.enigma.CommandMain <command>");
		System.out.println("\twhere <command> is one of:");
		System.out
			.println("\t\tdeobfuscate <in jar> <out jar> [<mappings file>]");
		System.out
	...
	private static void decompile(String[] args) throws Exception
	{
		...
		File fileJarOut =
			getWritableFolder(getArg(args, 2, "out folder", true));
		File fileMappings =
			getReadableFile(getArg(args, 3, "mappings file", false));
		...
		throws Exception
	{
		...
		if(fileMappings != null)
		{
			...
			Mappings mappings =
				new MappingsReader().read(new FileReader(fileMappings));
	...
	private static File getWritableFile(String path)
	{
		...
		if(dir == null || !dir.exists())
			throw new IllegalArgumentException("Cannot write to folder: "
				+ file);
		return file;
	}
